...
func coraza_rules_merge(w1 C.coraza_waf_t, w2 C.coraza_waf_t, er **C.char) C.int {
	h1 := fromRaw[*WafHandle](w1)
	h2 := fromRaw[*WafHandle](w2)
	rules1, ok := h1.waf.(experimental.WAFWithRules)
	if !ok {
		if er != nil {
			*er = C.CString("WAF does not support rule merging")
		}
		return 1
	}
	rules2, ok := h2.waf.(experimental.WAFWithRules)
	if !ok {
		if er != nil {
			*er = C.CString("WAF does not support rule merging")
		}
...
func coraza_free_waf(w C.coraza_waf_t) C.int {
	deleteRaw(w)
	return 0
}
